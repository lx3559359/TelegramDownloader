...
- 所有来源进入同一下载队列，同一时刻只执行一个任务；等待任务显示“等待中 · 第 N 位”，可对选中项执行“优先下载”。
- 设置页支持实时调整 1–5 个文件并发与总下载限速，修改后无需重启当前下载。
...
普通链接、账号内容搜索和自动订阅创建的任务统一按入队时间依次下载，同一时刻只会有一个活动任务，避免多个大型任务互相争抢连接和磁盘。等待项会显示“等待中 · 第 N 位”。选中一个等待任务后可点击“优先下载”，它会移动到当前活动任务之后；正在下载的任务不会被强制中断。优先顺序保存在任务数据库中，程序重启后继续生效。

设置中的“文件并发”控制当前活动任务内部同时下载的媒体文件数，可在 1–5 之间调整；“总下载限速”是所有媒体下载共享的总速率，而不是每个文件各自获得一份带宽。两项设置保存后立即作用于当前运行的调度器；不限速只取消应用自身的速率限制，不会改变 Telegram、网络或磁盘的实际上限。下载分片会在后台批量写盘，暂停、取消和完成时仍执行持久化提交，进度只统计已经确认写入的字节。等待队列、并发和限速设置都保存在应用目录，继续遵守不把业务数据写入 C 盘的边界。
