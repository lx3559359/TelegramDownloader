...
任务摘要使用一次 SQLite 聚合查询刷新，不再为列表中的每个任务分别打开数据库查询；只有单选任务时才读取该任务的媒体明细。任务数量增长后，搜索、状态切换和实时进度刷新仍保持本地响应。
...
设置中的“文件并发”控制当前活动任务内部同时下载的媒体文件数，可在 1–5 之间调整；“总下载限速”是所有媒体下载共享的总速率，而不是每个文件各自获得一份带宽。两项设置保存后立即作用于当前运行的调度器；不限速只取消应用自身的速率限制，不会改变 Telegram、网络或磁盘的实际上限。等待队列、并发和限速设置都保存在应用目录，继续遵守不把业务数据写入 C 盘的边界。
...
打开左侧“账号内容”后，搜索范围默认是“全部会话”。输入必填关键词，设置日期、媒体类型和 1–10,000 的账号级总结果上限，即可搜索当前账号可访问的群组、频道、私聊、机器人、收藏夹和已归档云端会话；也可以改选一个群组或频道，仅搜索该会话。程序会自动补齐稀疏的 Telegram 服务端分页，每次向界面提供最多 100 个新结果，尚有结果时可点击“加载更多”。例如 500 条结果会分多页取得；遇到不超过 120 秒的 Telegram 限流时显示逐秒倒计时并在同一游标自动重试，长等待或重试次数用尽时保留已有结果和游标，可稍后点击“继续搜索”。
